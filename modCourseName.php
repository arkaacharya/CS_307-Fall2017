				...
				<div id="content">
					...
					<?php
						...
						if(isset($_POST['courseName'])){
							echo $sql = "SELECT * FROM courses WHERE id='".$_POST['courseName'].$userName."'";
							$data = mysqli_query($conn, $sql);
							$result = mysqli_fetch_row($data);
							if(!$result){
								$sql = "UPDATE courses SET name='".$_POST['courseName']."', id='".$_POST['courseName'].$userName."' WHERE id='".$course.$userName."'";
								$data = mysqli_query($conn, $sql);
								
								$sql = "UPDATE ".$userName." SET course='".$_POST['courseName']."' WHERE course='".$course."'";
								$data = mysqli_query($conn, $sql);
								
								$sql = "ALTER TABLE ".preg_replace('/\s+/', '', $course)." RENAME TO ".preg_replace('/\s+/', '', $_POST['courseName']);
								$data = mysqli_query($conn, $sql); //Executing the sql query"
								
								header("Location: modCourseName.php?userName=".$userName."&course=".$course);
								die;
							}
							else{
								header("Location: modCourseName.php?failModName=true&userName=".$userName."&course=".$course);
								die;
							}
						}
